...
export function BenchmarkingUI({ benchmark }: BenchmarkingProps) {
  ...
  const comparison = benchmark.elite_comparison ?? benchmark.comparison
  const gapAnalysis = benchmark.gap_analysis
  const percentile = benchmark.percentile_estimate
  ...
  return (
    <Card className="border-primary/20 bg-primary/5">
      <CardHeader>
        ...
        <CardDescription>
          Automated comparison against an elite debate standard.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        {benchmark.userSegment && benchmark.eliteSegment && (
        ...
        <div className="bg-accent/50 p-4 rounded-md flex gap-4 items-start border">
          ...
          <div className="space-y-2">
            ...
            {gapAnalysis && <p className="text-sm leading-relaxed text-muted-foreground">{gapAnalysis}</p>}
          </div>
        </div>
      </CardContent>
    </Card>
